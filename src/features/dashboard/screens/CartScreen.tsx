...
// DON'T import RazorpayCheckout at top level — it crashes Expo Go
...
// Payment methods - Currently only COD is enabled, others are commented out
...
const CartItemRow: React.FC<{ item: { product: any; qty: number } }> = ({
  item,
}) => {
...
interface CartScreenProps {
  onTabPress?: (tab: CartTabName) => void;
}
...
export const CartScreen: React.FC<CartScreenProps> = ({ onTabPress }) => {
  ...
  const [paymentMethod, setPaymentMethod] = useState<string | null>(null); // No payment method selected by default
  const [ordering, setOrdering] = useState(false);
  const [orderResult, setOrderResult] = useState<MakeOrderResponse | null>(
    null,
  );
  ...
  const couponDiscount = (() => {
    if (!coupon) return 0;
    if (coupon.couponValueType === "PERCENTAGE")
  ...
  const handleProceedToPayment = async () => {
    ...
    if (paymentMethod === "COD") {
      // COD: directly place order without Razorpay
      setOrdering(true);
      try {
        ...
        console.log(
          "[CartScreen] COD makeOrder response:",
          JSON.stringify(res),
        );
        ...
        console.error("[CartScreen] COD makeOrder error:", e);
        Alert.alert(
          "Order Failed",
    ...
    // Online payment: Step 1 — create order on backend
    let makeOrderRes: MakeOrderResponse;
    try {
      makeOrderRes = await orderService.makeOrder({
        itemRequests: items.map((i) => ({
          productId: i.product.id,
          quantity: i.qty,
        })),
      ...
      console.log(
        "[CartScreen] makeOrder response:",
        JSON.stringify(makeOrderRes),
      );
    } catch (e: any) {
      console.error("[CartScreen] makeOrder error:", e?.response?.data ?? e);

      // Check if it's the Razorpay minimum amount error from backend
      const errorMsg = e?.response?.data?.message || e?.message || "";
      const isRazorpayMinAmountError = errorMsg.includes(
        "Order amount less than minimum amount allowed",
      );

      if (isRazorpayMinAmountError) {
        Alert.alert(
          "Payment Gateway Error",
          `Razorpay minimum order amount is ₹100.\n\nYour total: ₹${totalAmount}\n\n⚠️ Backend issue: Backend is sending amount in rupees instead of paise to Razorpay.\n\nWorkaround:\n• Use "Cash on Delivery" to place order now\n• Or add more items (total ≥ ₹100)`,
          [
            {
              text: "Switch to COD",
              onPress: () => {
    ...
    if (makeOrderRes.status !== "AVAILABLE" || !makeOrderRes.paymentRes) {
      console.warn(
        "[CartScreen] makeOrder not AVAILABLE or no paymentRes:",
        makeOrderRes,
      );
      Alert.alert(
        "Order Failed",
        makeOrderRes.message || "Payment details not available.",
      );
      return;
    }

    const { razorpayKey, razorpayOrderResponse } = makeOrderRes.paymentRes;
    console.log(
      "[CartScreen] razorpayKey:",
      razorpayKey,
      "| razorpay order id:",
      razorpayOrderResponse?.id,
    );

    // Guard: native module check (fails silently in Expo Go)
    const { NativeModules } = require("react-native");
    if (!NativeModules.RNRazorpayCheckout) {
      console.error(
        "[CartScreen] RNRazorpayCheckout native module NOT found. Run: npx expo run:android / run:ios",
      );
      Alert.alert(
        "Razorpay Not Available",
        'Payment gateway requires a native build.\n\nRun: npx expo run:android\n\nOr select "Cash on Delivery" to place order now.',
        [
          {
    ...
    // Backend sends amount in rupees (e.g. totalAmount=199 → backend sends 199)
    // Razorpay needs paise (19900). Always multiply by 100.
    const amountInPaise = razorpayOrderResponse.amount * 100;

    console.log(
      `[CartScreen] Converting amount: backend=${razorpayOrderResponse.amount} → paise=${amountInPaise}`,
    );

    const options = {
      description: "KhanaMart Order Payment",
    ...
    try {
      console.log(
        "[CartScreen] Opening Razorpay with options:",
        JSON.stringify(options),
      );

      // Dynamic import so it doesn't crash Expo Go on screen load
      const RazorpayCheckout = require("react-native-razorpay").default;
      const paymentData = await RazorpayCheckout.open(options);

      console.log(
        "[CartScreen] Razorpay success:",
        JSON.stringify(paymentData),
      );

      // Show loading after Razorpay response
      setOrdering(true);

      // Step 3A — Save payment success to backend
      try {
        await orderService.savePayment(makeOrderRes.orderNumber, {
          razorpayOrderId: paymentData.razorpay_order_id,
          razorpayPaymentId: paymentData.razorpay_payment_id,
          razorpaySignature: paymentData.razorpay_signature,
          paymentStatus: "SUCCESS",
        });
        console.log("[CartScreen] Payment success saved to backend");
      } catch (saveErr) {
        console.error(
          "[CartScreen] savePayment error (non-blocking):",
          saveErr,
        );
      }

      // Step 3B — Verify payment with backend
      try {
        ...
        console.log(
          "[CartScreen] verifyPayment response:",
        ...
        if (verifyRes.status === "AVAILABLE") {
          ...
          setOrderResult({
            ...makeOrderRes,
            orderNumber: verifyRes.orderNumber || makeOrderRes.orderNumber,
          });
        } else {
          Alert.alert(
            "Verification Failed",
        ...
        console.error(
          "[CartScreen] verifyPayment error:",
          e?.response?.data ?? e,
        );
        // Payment captured but verification call failed — still show success
        clearCart();
        setOrderResult(makeOrderRes);
      }
    } catch (error: any) {
      console.error("[CartScreen] Razorpay error:", JSON.stringify(error));
      ...
      // Show loading for failure case (except user cancellation)
      if (code !== 0) {
        setOrdering(true);
      }

      // Save payment failure to backend
      if (code !== 0) {
        try {
          await orderService.savePayment(makeOrderRes.orderNumber, {
          ...
          console.log("[CartScreen] Payment failure saved to backend");
        } catch (saveErr) {
          console.error(
            "[CartScreen] savePayment failure error (non-blocking):",
            saveErr,
          );
        }
      ...
      // code 0 = user manually dismissed the sheet — no alert needed
    } finally {
      setOrdering(false);
    }
  };
      ...
      >
        ...
        <View style={S.sectionCard}>
          ...
          >
            ...
            <Ionicons
              name={couponOpen ? "chevron-up" : "chevron-down"}
              size={18}
              color="#6B7280"
            />
          </TouchableOpacity>
            ...
            <>
              {coupon ? (
                <View style={S.couponApplied}>
                  <View style={{ flex: 1 }}>
                    <Text style={S.couponAppliedCode}>
                      "{coupon.code}" Applied!
                    </Text>
        ...
        <View style={S.sectionCard}>
          <View style={S.addressRow}>
            ...
            <View style={{ flex: 1, marginLeft: 8 }}>
              ...
              <Text style={S.addressSub} numberOfLines={2}>
                {addressLine}
              </Text>
            </View>
            ...
            return (
              ...
              >
                <View
                  style={[
                    S.paymentIconWrap,
                    selected && S.paymentIconWrapActive,
                  ]}
                >
                  <Ionicons
                    name={pm.icon}
                ...
                <View style={{ flex: 1 }}>
                  <Text
                    style={[S.paymentLabel, selected && S.paymentLabelActive]}
                  >
                    {pm.label}
                  </Text>
        ...
        <View style={S.summaryCard}>
          ...
          <View style={S.summaryRow}>
            ...
            <Text
              style={[S.summaryValue, { color: "#16A34A", fontWeight: "700" }]}
            >
              FREE
            </Text>
          </View>
