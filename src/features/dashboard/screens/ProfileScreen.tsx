...
import { CustomerAddress, addressService } from "../services/addressService";
import {
  CustomerOrder,
  OrdersResponse,
  profileService,
...
// ─── ADDRESS TYPES ────────────────────────────────────────────────────────────
const ADDRESS_TYPES: Array<"HOME" | "OFFICE" | "OTHER"> = [
  "HOME",
  "OFFICE",
  "OTHER",
];
...
const AddressModal: React.FC<{
  ...
  onSaved?: () => void;
}> = ({ visible, onClose, initialAddress, onSaved }) => {
  ...
  // Pre-fill: if editing use initialAddress, else blank
  useEffect(() => {
    if (!visible) return;
    const a = initialAddress?.address ?? null;
    ...
    try {
      const requestData = {
        ...
        address: {
          ...
          latitude: initialAddress?.address?.latitude ?? "",
          longitude: initialAddress?.address?.longitude ?? "",
          type: addrType,
        },
      };
      ...
      if (initialAddress?.id) {
        ...
        await addressService.updateAddress(initialAddress.id, requestData);
      ...
      Alert.alert(
        "Error",
        e?.response?.data?.message ?? "Failed to save address",
      );
    } finally {
      setLoading(false);
    }
      ...
      >
        <View style={[S.modalSheet, { maxHeight: "92%" }]}>
          ...
          <View style={S.addrModalOwner}>
            ...
            <View style={{ flex: 1 }}>
              <Text style={S.addrModalOwnerName}>
                {profile?.fullName ?? ""}
              </Text>
              <Text style={S.addrModalOwnerPhone}>
          ...
          >
            ...
            <FormField
              ...
              placeholder="e.g. Gautam Budh Nagar"
            />
            <FormField
              label="City *"
              value={city}
              ...
              {ADDRESS_TYPES.map((t) => (
                ...
                >
                  <Text
                    style={[
                      S.typeChipTxt,
                      addrType === t && S.typeChipTxtActive,
                    ]}
                  >
                    {t}
                  </Text>
...
const DeliveryAddresses: React.FC<{ onBack: () => void }> = ({ onBack }) => {
  ...
  const loadAddresses = async () => {
    ...
    try {
      ...
      console.error("[ProfileScreen] Failed to load addresses:", e);
    } finally {
      setLoading(false);
    }
  };
        ...
        <View style={S.card}>
          <View style={S.cardRow}>
            ...
            <View style={S.savedBadge}>
              <Text style={S.savedBadgeTxt}>{addresses.length} saved</Text>
            </View>
          </View>
              ...
              <View key={addr.id}>
                ...
                <View style={S.addrItem}>
                  ...
                  <View style={S.addrActions}>
                    ...
                    <TouchableOpacity
                      style={[S.addrEditBtn, { marginLeft: 12 }]}
                    ...
                    >
                      <Ionicons
                        ...
                        color="#DC2626"
                      />
                      <Text style={[S.addrEditBtnTxt, { color: "#DC2626" }]}>
                        Delete
                      </Text>
                    </TouchableOpacity>
                  </View>
      ...
      >
        <View style={[S.modalSheet, { maxHeight: "92%" }]}>
          ...
          >
            ...
            {/* ── Address ── */}
            <Text style={[S.modalSectionLabel, { marginTop: 8 }]}>
              Delivery Address
            </Text>
      ...
      <ScrollView contentContainerStyle={{ paddingBottom: 40 }}>
        ...
        <View style={S.card}>
          ...
          <ToggleRow
            ...
            sub="Real-time delivery tracking"
            val={deliveryAlerts}
          ...
          <ToggleRow
            ...
            sub="Deals, discounts, and coupons"
            val={promotions}
        ...
        <View style={[S.card, { marginTop: 12 }]}>
          ...
          <ToggleRow
            ...
            val={locationAccess}
            onToggle={() => setLocationAccess((v) => !v)}
          />
        </View>
      </ScrollView>
    </SafeAreaView>
    ...
    <SafeAreaView style={S.safe} edges={["top"]}>
      ...
      <ScrollView contentContainerStyle={{ paddingBottom: 40 }}>
        ...
        <Text style={S.sectionSubtitle}>
          Where you can change the payment option
        </Text>
...
> = {
  PENDING: { label: "Pending", bg: "#FEF9C3", color: "#D97706" },
...
const OrderHistoryScreen: React.FC<OrderHistoryScreenProps> = ({ onBack }) => {
  ...
  const loadOrders = async (pageNum: number, replace: boolean) => {
    try {
      ...
      const res: OrdersResponse = await profileService.getOrders({
        page: pageNum,
        size: 10,
      });
  ...
  const handleLoadMore = () => {
    if (!isLast && !loadingMore && !loading) {
      loadOrders(page + 1, false);
    }
  };

  const statusFor = (status: string) =>
    STATUS_CONFIG[status] ?? { label: status, bg: "#F3F4F6", color: "#6B7280" };

  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: "#F9FAFB" }}>
      {/* Header */}
      <View style={SOH.header}>
        <TouchableOpacity
          onPress={onBack}
          style={SOH.backBtn}
        ...
        <Text style={SOH.title}>Order History</Text>
        <View style={{ width: 36 }} />
      </View>

      {/* Loading */}
      {loading && (
        <View style={SOH.center}>
      ...
      {!loading && error && (
        <View style={SOH.center}>
          ...
          <Text style={SOH.emptyTitle}>Something went wrong</Text>
          <Text style={SOH.emptySub}>{error}</Text>
          <TouchableOpacity
            style={SOH.retryBtn}
            onPress={() => loadOrders(0, true)}
          >
            <Text style={SOH.retryTxt}>Retry</Text>
          </TouchableOpacity>
        </View>
      )}
      ...
      {!loading && !error && orders.length === 0 && (
        <View style={SOH.center}>
          <Ionicons name="bag-outline" size={56} color="#D1FAE5" />
          <Text style={SOH.emptyTitle}>No orders yet</Text>
          <Text style={SOH.emptySub}>Your order history will appear here</Text>
        </View>
      )}
      ...
      {!loading && !error && orders.length > 0 && (
        ...
        >
          {orders.map((order) => {
            const st = statusFor(order.orderStatus);
            const itemSummary = order.items
              .map((i) => i.product.name)
              .join(", ");
            return (
              <View key={order.id} style={SOH.card}>
                {/* Row 1: Order # + Status badge */}
                <View style={SOH.cardRow}>
                  <Text style={SOH.orderNum}>{order.orderNumber}</Text>
                  <View style={[SOH.badge, { backgroundColor: st.bg }]}>
                    <Text style={[SOH.badgeTxt, { color: st.color }]}>
                      {st.label}
                    </Text>
                  </View>
                </View>
                ...
                {/* Row 2: Item summary */}
                <Text style={SOH.itemSummary} numberOfLines={2}>
                  {itemSummary}
                </Text>

                {/* Row 3: Date + Amount */}
                <View style={[SOH.cardRow, { marginTop: 8 }]}>
                  <Text style={SOH.dateText}>
                    {formatOrderDate(order.created)}
                  </Text>
                  <Text style={SOH.amountText}>
                    ₹{order.finalAmount.toFixed(2)}
                  </Text>
                </View>

                <View style={SOH.divider} />

                {/* Row 4: Reorder + Details */}
                <View style={SOH.cardRow}>
                  <TouchableOpacity
                    style={SOH.reorderBtn}
                    onPress={() =>
                      Alert.alert("Reorder", `Reorder ${order.orderNumber}?`)
                    }
                    activeOpacity={0.75}
                  >
                    <Ionicons
                      ...
                      color="#6B7280"
                    />
                    <Text style={SOH.reorderTxt}> Reorder</Text>
                  </TouchableOpacity>

                  <TouchableOpacity
                    style={SOH.detailsBtn}
                    onPress={() =>
                      Alert.alert(
                    ...
                    activeOpacity={0.75}
                  >
                    <Text style={SOH.detailsTxt}>Details</Text>
                    <Ionicons
...
const MenuRow: React.FC<{
  ...
  <TouchableOpacity style={S.menuRow} onPress={action} activeOpacity={0.7}>
    ...
    <View style={{ flex: 1 }}>
      ...
      {!!sub && (
        <Text style={S.menuSub} numberOfLines={2}>
          {sub}
        </Text>
      )}
    </View>
...
export const ProfileScreen: React.FC<ProfileScreenProps> = ({ onTabPress }) => {
  ...
  // Sub-screen routing
  if (subScreen === "delivery-addresses")
    return <DeliveryAddresses onBack={() => setSubScreen("main")} />;
  if (subScreen === "notifications")
    return <NotificationsScreen onBack={() => setSubScreen("main")} />;
  if (subScreen === "payment-methods")
    return <PaymentMethodsScreen onBack={() => setSubScreen("main")} />;
  if (subScreen === "change-password")
    return <ChangePasswordScreen onBack={() => setSubScreen("main")} />;
  if (subScreen === "order-history")
    return <OrderHistoryScreen onBack={() => setSubScreen("main")} />;

  const displayName = profile?.fullName ?? "User";
  // Use profile creation year if available, else current year
  const memberYear = profile?.id
  ...
  const MENU_ACCOUNT = [
    {
      label: "Order History",
      sub: "View past orders & reorder",
      icon: "bag-check-outline",
      action: () => setSubScreen("order-history"),
    },
    {
      label: "Payment Methods",
      sub: "Saved cards, UPI, wallet",
      icon: "card-outline",
      action: () => setSubScreen("payment-methods"),
    },
    {
      label: "Delivery Addresses",
      sub: "Add/edit/delete addresses",
      icon: "location-outline",
      action: () => setSubScreen("delivery-addresses"),
    },
    {
      label: "Coupons & Offers",
      sub: "Available discount coupons",
      icon: "ticket-outline",
      action: () => Alert.alert("Coming Soon", "Coupons"),
    },
    {
      label: "Subscription Plans",
      sub: "Active & past subscriptions",
      icon: "calendar-outline",
      action: () => onTabPress?.("Subscription"),
    },
  ];
  ...
  const MENU_REVIEWS = [
    {
      ...
      sub: "View your ratings and feedback",
      icon: "star-outline",
      action: () => Alert.alert("Coming Soon", "Reviews"),
    },
  ];
  ...
  const MENU_SECURITY = [
    {
      ...
      action: () => setSubScreen("change-password"),
    },
    {
    ...
    },
    {
      ...
      action: () => Alert.alert("Coming Soon", "FAQs"),
    },
  ];
  ...
  return (
    <SafeAreaView style={S.safe} edges={["top"]}>
      ...
      >
          ...
          <View style={{ flex: 1 }}>
            {isLoading ? (
              ...
              <>
                ...
                <Text style={S.profileMember}>
                  Member since Jan {memberYear}
                </Text>
              </>
            )}
          </View>
        ...
        {/* Favourites — coming soon, no API yet */}
        <View style={[S.card, { marginTop: 12 }]}>
          <View style={S.cardRow}>
            <View style={{ flexDirection: "row", alignItems: "center" }}>
              <Ionicons name="heart" size={18} color="#EF4444" />
              <Text style={[S.cardBoldTitle, { marginLeft: 6 }]}>
                My Favourites
              </Text>
            </View>
          </View>
